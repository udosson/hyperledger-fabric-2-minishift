...
CC_SRC_PATH="/fabric/chaincode/fabcar/go/"
...
approveForMyOrg() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ] ; then
    set -x
    peer lifecycle chaincode approveformyorg -o orderer:31010  --channelID $CHANNEL_NAME --name fabcar --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${SEQUENCE} --waitForEvent >&log.txt
    set +x
  else
    set -x
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name fabcar --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA $PEER_CONN_PARMS --version ${VERSION} --sequence ${SEQUENCE} --output json --init-required >&log.txt
    res=$?
    set +x
...
commitChaincodeDefinition() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ] ; then
    set -x
    peer lifecycle chaincode commit -o orderer:31010 --channelID $CHANNEL_NAME --name fabcar $PEER_CONN_PARMS --version ${VERSION} --sequence ${SEQUENCE} --init-required >&log.txt
    res=$?
    set +x
  else
    set -x
...
chaincodeInvokeInit() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer:31010 -C $CHANNEL_NAME -n fabcar $PEER_CONN_PARMS --isInit -c '{"function":"init","Args":[]}' >&log.txt
    res=$?
    set +x
  else
    set -x
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer:31010 -C $CHANNEL_NAME -n fabcar $PEER_CONN_PARMS  -c '{"function":"initLedger","Args":[]}' >&log.txt
    res=$?
    set +x
  else
    set -x
